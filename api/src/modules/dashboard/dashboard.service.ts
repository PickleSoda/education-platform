...
export const getTeacherDashboard = async (teacherId: string) => {
	...
	const totalStudents = activeInstances.reduce((sum, instance) => sum + instance._count.enrollments, 0);
...
export const getInstanceAnalytics = async (instanceId: string, assignmentId?: string) => {
	const gradeDistribution = await dashboardRepository.getInstanceGradeDistribution(instanceId, assignmentId);
